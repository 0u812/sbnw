...
//     printf("gfp_NetworkRandomizeLayout called\n");
    ...
    if(Py_TYPE(canvas) != &gfp_CanvasType) {
        PyErr_SetString(PyExc_TypeError, "Expected graphfab.canvas type");
        return NULL;
    }
...
static PyTypeObject gfp_CubicintersecType = {
    ...
    0,
#endif
    "graphfab.cubicintersec",      /* tp_name */
...
static PyTypeObject gfp_LayoutType = {
    ...
    ":meth:`graphfab.network.randomize`.",
    0,                         /* tp_traverse */
    0,                         /* tp_clear */
    0,                         /* tp_richcompare */
    0,                         /* tp_weaklistoffset */
...
PyObject* gfp_SBMLModel_save(gfp_SBMLModel *self, PyObject *args, PyObject *kwds) {
    ...
    if(!PyArg_ParseTupleAndKeywords(args, kwds, "s", kwlist, &outfile)) {
        PyErr_SetString(GraphfabError, "Invalid arguments to graphfab.model.save; expected filepath string");
        return NULL;
    }
...
static PyMethodDef GraphfabMethods[] = {
    ...
    {"loadsbml",  (PyCFunction)gfp_loadsbml, METH_VARARGS | METH_KEYWORDS,
     ...
     ">>> model = graphfab.loadsbml('path/to/file/sbmlmodel.xml')\n\n"
     ":param sbml: Either a filepath or raw SBML content to load\n"
     ":type sbml: str\n"
     ":returns: :class:`graphfab.sbmlmodel` -- The SBML moodel\n"
...
#endif
{
    ...
    GraphfabError = PyErr_NewException("graphfab.error", NULL, NULL);
